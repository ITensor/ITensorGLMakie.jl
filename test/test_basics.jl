...
@testset "Basic test for ITensorGLMakie" begin
    ...
    #   reasons we haven't localized. A PSNR check against refs captured on
    #   1.11+ would spuriously fail on 1.10 even when the render pipeline
    #   is healthy. We still exercise the full `@visualize` → render → save
    #   path; the visual regression gate is on 1.11+ only. Mirrors
    #   GraphMakie.jl's `if VERSION < v"1.11"; MEH = 0; end`.
    by = if extension != "png"
        isequal
